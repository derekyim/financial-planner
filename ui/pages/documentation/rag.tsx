...
import { APP_NAME, EMBEDDING_MODEL, EMBEDDING_DIMS, CHUNK_SIZE, CHUNK_OVERLAP, RETRIEVAL_K } from '@/constants';
import styles from '@/styles/content.module.css';

const DATA_SOURCES = [
  { category: 'Financial Glossary', description: 'Business and financial terms', file: 'data/financial_glossary/' },
...
export default function RagDocPage() {
  return (
    <>
      <Head>
        <title>{`RAG Application | ${APP_NAME}`}</title>
      </Head>
      <Box className={styles.page}>
        ...
        <Paper className={styles.card} elevation={0}>
          ...
          <Typography className={styles.body}>
            We use <strong>RecursiveCharacterTextSplitter</strong> with a chunk size of {CHUNK_SIZE} characters
            and {CHUNK_OVERLAP}-character overlap. This was chosen because:
          </Typography>
          <ul className={styles.list}>
            <li className={styles.listItem}>
              Business knowledge documents contain short, self-contained paragraphs and bullet points &mdash;
              {CHUNK_SIZE} characters captures a complete concept without splitting mid-sentence.
            </li>
            <li className={styles.listItem}>
              The {CHUNK_OVERLAP}-character overlap ensures continuity at chunk boundaries, preventing loss of
          ...
          <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mt: 1 }}>
            <Chip label={`chunk_size=${CHUNK_SIZE}`} variant="outlined" size="small" />
            <Chip label={`chunk_overlap=${CHUNK_OVERLAP}`} variant="outlined" size="small" />
            <Chip label={`k=${RETRIEVAL_K} retrieval`} variant="outlined" size="small" />
          </Box>
        </Paper>
        ...
        <Paper className={styles.card} elevation={0}>
          ...
          <Typography className={styles.body}>
            ...
            These are embedded using <strong>{EMBEDDING_MODEL}</strong> (OpenAI, {EMBEDDING_DIMS} dimensions)
        ...
        <Paper className={styles.card} elevation={0}>
          ...
          <ol className={styles.list}>
            ...
            <li className={styles.listItem}>Agent calls <code>{`retrieve_context(query, k=${RETRIEVAL_K})`}</code> on Qdrant</li>
