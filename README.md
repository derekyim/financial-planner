...
This project is configured for monorepo deployment on Vercel — the Next.js frontend and the FastAPI backend both deploy from a single repo using `vercel.json`.

### How It Works

`vercel.json` maps routes to two build targets:

- **Frontend** (`ui/`) — built with `@vercel/next`, serves all non-API routes
- **Backend** (`backend/api/index.py`) — built with `@vercel/python` as a serverless function, handles `/api/*` routes

The frontend's `chatService.ts` auto-detects the environment: on Vercel it calls `/api/...` (same origin), locally it calls `http://localhost:8000`.

### Step 1: Connect the Repo

1. Go to [vercel.com/new](https://vercel.com/new) and import the `financial-planner` GitHub repo
2. Vercel will detect the `vercel.json` and configure the build automatically
3. Set the **Root Directory** to `.` (the repo root, not `ui/`)
...
| `SPREADSHEET_URL` | Yes | Default Google Sheets URL for the financial model |
| `GOOGLE_CREDENTIALS_PATH` | Yes | Path to the Google service account JSON (see note below) |
| `LANGCHAIN_API_KEY` | No | LangSmith API key for tracing |
| `LANGCHAIN_TRACING_V2` | No | Set to `true` to enable LangSmith tracing |
| `LANGSMITH_API_KEY` | No | LangSmith API key |
| `ADVANCED_RETRIEVAL` | No | Set to `true` for hybrid BM25+dense retrieval |
| `NEXT_PUBLIC_API_URL` | No | Override the API base URL (usually not needed on Vercel) |

### Step 3: Handle Google Credentials

The `credentials.json` file is gitignored and won't be available on Vercel. You have two options:

1. Base64-encode your credentials file locally:
   ```bash
   base64 -i backend/credentials.json | tr -d '\n'
   ```
2. Add the output as a Vercel env var named `GOOGLE_CREDENTIALS_B64`
3. Add a Vercel build command or modify `backend/api/index.py` to decode it at startup:
   ```python
...
Alternatively, just push to `main` — Vercel auto-deploys on every push if the GitHub integration is connected.

### Troubleshooting

- **Python dependency errors** — Vercel looks for `requirements.txt` in the repo root for `@vercel/python` builds. The current `requirements.txt` is already at the root.
- **API 500 errors** — Check Vercel Function Logs (Project > Deployments > Functions tab). Common causes: missing env vars or Google Sheets not shared with the service account email.
- **CORS issues** — The frontend and backend are on the same Vercel domain, so CORS should not be an issue. If testing locally against a Vercel deployment, set `NEXT_PUBLIC_API_URL` to the Vercel URL.
- **Cold starts** — The Python serverless function may have a ~2-3 second cold start on the first request. Subsequent requests within the warm window are faster.
